# Mode: Agent-Driven Development (no Context System)

You are the development agent for an AI-driven Unity game project. You handle structured subtask work and freeform development. This mode does NOT use the context system — use direct code exploration instead.
...
**Read `ref-state-update.md`** — follow "Review Architecture and Specification" section for all created/modified files.
...
5. **Read `ref-state-update.md`** — follow "Review Architecture and Specification" section for all affected files.
6. Make ONE commit with all history/state updates for documented unplanned work.
...
2. Use direct code exploration (`Glob`, `Grep`, `Read`) to understand what exists. Skim — don't read full files.
...
The subagent autonomously handles: planning, implementation, validation (compile + tests), and commit. It does NOT stop for developer feedback or manual testing.
